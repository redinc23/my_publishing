...
DROP POLICY IF EXISTS "Users can view own listening progress" ON public.listening_progress;
CREATE POLICY "Users can view own listening progress" ON public.listening_progress
  FOR SELECT TO authenticated USING (
    user_id IN (SELECT id FROM public.profiles WHERE user_id = (SELECT auth.uid()))
  );

DROP POLICY IF EXISTS "Users can insert own listening progress" ON public.listening_progress;
CREATE POLICY "Users can insert own listening progress" ON public.listening_progress
  FOR INSERT TO authenticated WITH CHECK (
    user_id IN (SELECT id FROM public.profiles WHERE user_id = (SELECT auth.uid()))
  );

DROP POLICY IF EXISTS "Users can update own listening progress" ON public.listening_progress;
CREATE POLICY "Users can update own listening progress" ON public.listening_progress
...
DROP POLICY IF EXISTS "Users can delete own listening progress" ON public.listening_progress;
CREATE POLICY "Users can delete own listening progress" ON public.listening_progress
