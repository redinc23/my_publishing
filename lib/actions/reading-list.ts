...
export async function addToReadingList(bookId: string, status: ReadingStatus = 'want_to_read') {
  ...
  if (existing) {
    ...
    // Create new entry
    result = await supabase.from('reading_lists').insert(data).select().single();
  }
...
export async function getReadingList(userId: string, status?: ReadingStatus, page = 1, limit = 20) {
  const supabase = await createClient();
    ...
    books:
      data?.map((item) => ({
        ...
        book: {
          ...item.book,
          author_name:
            item.book.author?.full_name || item.book.author?.username || 'Unknown Author',
        },
      })) || [],
    total: count || 0,
    page,
