...
**Document Version:** 4.0.1 (Implementation-Locked Edition + Phase 0 recon amendments)  
...
**Phase 0 recon:** `docs/PHOENIX_RECON.md` (deep dive on `main` @ `9320407`)
...
>
> **v4.0.1 (Phase 0):** Amended RBAC to match live product (`partner`, not `editor`);
> env URL var → `NEXT_PUBLIC_SITE_URL`; Mongo client filename note; field-name mapping
> for ratings/payment intent; feature-freeze clarification for under-specified surfaces.
> See `docs/PHOENIX_RECON.md` §10.
...
- **Deferred post-Phoenix (live in repo but not in Mongo target set unless a WS PR explicitly adds them):** Resonance/pgvector embeddings, MCP transport, social graph beyond reviews (follows/comments/reading_lists), author payouts / `book_sales` ledger, subscriptions/notifications tables. Partner portal + ARC **are in scope for auth/RBAC parity** (role = `partner`). Comics/papers via `books.content_type` remain in catalog parity.
...
- **R-ADMIN-01:** Role-based access control with four tiers: `reader`, `author`, `partner`, `admin` (matches live Postgres CHECK and partner portal; do **not** introduce an `editor` role during Phoenix).
...
| `books`                  | `books`            | Application | `author_id`, `slug`                         | Convert `author_id` to ObjectId. Generate URL-safe unique `slug` from title. Map live `average_rating`→`avg_rating`, prefer `review_count` (fallback `total_reviews`). Preserve `content_type` (`book`\|`comic`\|`paper`). Initialize ratings to 0 if absent. Rewrite `cover_url`/`manuscript_url` to Vercel Blob (WS3 Task 3.4). |
| `orders` + `order_items` | `orders`           | Application | `order_items[]`, `stripe_payment_intent_id` | Flatten join table into embedded `order_items[]` array. Map live column `orders.payment_intent_id` → Mongo `stripe_payment_intent_id`. **Add unique sparse index** (live Postgres lacks UNIQUE on this column — event-level idempotency only via `webhook_events`). |
...
| **1.4** | **Rewrite Middleware**           | 1.4.1 Parse Better Auth session cookie (**cookie-only — no Mongo on Edge**) <br> 1.4.2 Define public vs protected route matcher (`/dashboard*`, `/admin*`, `/author*`, `/partner*`, `/library*`, `/reading*`, `/api/files*`) <br> 1.4.3 Implement role-based redirects (`/admin` → admin only; `/author` → author\|admin; `/partner` → partner\|admin) <br> 1.4.4 Graceful redirect to `/login?next=<path>` with return URL                                                                | `middleware.ts`                                                | Unauthenticated `/dashboard` redirects to `/login?next=…`           |
...
| Task #   | Action                      | Subtasks                                                                                                                                                                                                                                                                                                                                             | Files                                  | Verification                                                                   |
| -------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------ |
| **2a.1** | **MongoDB Connection**      | 2a.1.1 Install `mongodb` driver (or adopt scaffold) <br> 2a.1.2 Create global singleton cached client <br> 2a.1.3 Implement connection error handling <br> 2a.1.4 **Filename:** prefer existing scaffold `lib/mongodb.ts` exporting `getDb()` (doc historically said `lib/mongo.ts` — either path OK if `getDb()` is the public API) | `lib/mongodb.ts` (or `lib/mongo.ts`) | `getDb()` returns valid `Db` instance                                          |
...
| `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Stripe     | ✅       | `pk_live_...` / `pk_test_...` (already wired in repo)                 |
...
| `SENTRY_ORG` / `SENTRY_PROJECT` / `SENTRY_AUTH_TOKEN` | Sentry | ⚠️ CI | Source maps upload (WS6)                                      |
| `NEXT_PUBLIC_SITE_URL`      | App URL           | ✅       | Canonical app URL (**repo name** — do not rename to `NEXT_PUBLIC_APP_URL`) |
| `OPENAI_API_KEY`            | OpenAI            | ⚠️ Opt   | Resonance / AI insights (deferred features may keep optional)         |
...
| `lib/mongodb.ts`             | MongoDB connection singleton (`getDb()`) | WS2a       | ✅ New / adopt scaffold |
...
| 4.0.1   | 2026-07-18 | Phase 0 recon | RBAC `editor` → `partner` (live product); env `NEXT_PUBLIC_SITE_URL`; Mongo client `lib/mongodb.ts` + `getDb()`; §1.4 deferred surfaces; middleware route list; Stripe publishable + Sentry CI vars in §9.1. See `docs/PHOENIX_RECON.md`. |
