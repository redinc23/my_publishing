...
| D10 | **WS2b dual-run:** `DATABASE_PROVIDER=supabase\|mongodb` (default `supabase`). Books/checkout/webhook API routes branch on `isMongoPrimary()`; Mongo webhook upserts by `stripe_payment_intent_id` (`lib/mongo-orders.ts`, `lib/stripe/webhook-mongo.ts`). Canonical webhook path remains `/api/webhook` (+ `/api/webhooks/stripe` re-export). |
...
| `app/api/books/route.ts`      | Books list/create (dual-run)      | WS2b       | ✅ New                                             |
| `app/api/books/[id]/route.ts` | Book get/patch (dual-run)         | WS2b       | ✅ New                                             |
| `lib/mongo-orders.ts`         | Idempotent order upsert           | WS2b       | ✅ New                                             |
| `lib/stripe/webhook-mongo.ts` | Mongo Stripe fulfillment          | WS2b       | ✅ New                                             |
| `lib/api/request-user.ts`     | Dual auth for API routes          | WS2b       | ✅ New                                             |
