...
Evidence: stub surface under `app/(consumer)/book-clubs` + `discover/book-clubs`; G6 no false-success
...
Gate/Star tie: G6
Effort: S · Risk: none · Verification: visual QA on `/book-clubs` and `/discover/book-clubs`
Approval: not required (L0)
Shipped: 2026-07-20 — explicit “not available yet” status; metadata no longer implies live join; no fake create/waitlist CTAs; cards removed in favor of plain resource links
