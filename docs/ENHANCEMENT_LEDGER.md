...
Shipped: 2026-07-20 — copy clarifies clubs are not a live product; CTAs point only to working routes.
...
### E-006 MCP catalog dual-run + smarter recommend

Lane: L1 · Status: BUILDING · Score: R4 I4 C5 /E2 = 40
Story: As an MCP client (Cursor/Copilot), I get the same public catalog tools whether the app is on Supabase or Mongo, with better ranking and optional similar-to seeding.
Evidence: enhance request “website tool”; phoenix-mcp-migration checklist; WS2a query layer on main
Metric & target: MCP tools stable on both providers; unit tests for catalog + mongo helpers green
Flag: n/a (provider switch already gated by DATABASE_PROVIDER; prod stays supabase)
Gate/Star tie: Phoenix MCP migration / North Star data-layer readiness
Effort: M · Risk: response shape drift · Verification: `npx jest tests/unit/mcp-catalog.test.ts tests/unit/mcp-transport-security.test.ts tests/unit/mongo-queries.test.ts`
Approval: not required (migration parity + hardening)
