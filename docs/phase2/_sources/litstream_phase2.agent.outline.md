...
#### 2.4.1 VITE_ prefix rule: variables starting with VITE_ are inlined into client JS; secret variables MUST NOT use this prefix (table: which vars use VITE_ vs which don't)
...
#### 3.3.2 Public vars (VITE_ prefix): VITE_SANITY_PROJECT_ID, VITE_SANITY_DATASET, VITE_SANITY_API_VERSION, VITE_SITE_URL, VITE_APP_VERSION — safe for client bundles
#### 3.3.3 Secret vars (NO VITE_ prefix): SANITY_API_READ_TOKEN (Secret Manager only), SENTRY_AUTH_TOKEN (optional)
...
#### 3.4.2 ADR-002: No VITE_ prefix on SANITY_API_READ_TOKEN — prevents Vite from inlining the secret into client JS; token only accessible to Node.js build scripts
...
#### 7.1.2 R1: Secret leakage via VITE_ prefix — mitigation: audit:secrets script, Zod validation, grep checks
