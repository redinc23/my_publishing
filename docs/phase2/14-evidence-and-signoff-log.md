...
| Field | Value |
|---|---|
| Entry ID | PENDING *(e.g. yyyymmdd-001)* |
...
| Operator | PENDING |
| Known Good Revision | PENDING *(should match `KNOWN_GOOD_REVISION` in `_intake/environment.local.sh`)* |
...
|---|---|---|---|---|
| Engineering Lead | PENDING *(must match `12` Primary)* | GO / NO-GO | PENDING | |
| Platform Lead | PENDING *(must match `12` Primary)* | GO / NO-GO | PENDING | |
| Security Lead | PENDING *(must match `12` Primary)* | GO / NO-GO | PENDING | |
| Product/Business Owner | PENDING *(must match `12` Primary)* | GO / NO-GO | PENDING | |
