...
| **Established by**           | Phase 1 authority PR #206 (`0f30649`) → **G13 TRUE**. v1.2.2: Phase 4 Tier L (#226), Phase 5 CI hygiene (#225), Phase 6 ADR-001 Option A **RECOMMENDED** + monitor retarget (P0-003/P0-007; G9 still FALSE pending signatures). |
...
| CI on `main` `5744794` (#223)             | **GREEN** (VERIFIED 2026-07-18)              | Actions: CI + Format Check + CodeQL + Lighthouse + Playwright E2E + Release Please all **success** on https://github.com/redinc23/my_publishing/actions/runs/29629590731. Prior Format Check RED / MCP route-export RED SUPERSEDED. Tip `a0a9cf5` = code tip + bug-to-issue state chores (bot pushes; no re-trigger). G2 still FALSE until green on the **deployed release** SHA (Phase 14) |
...
| 6   | Canonical Platform Decision & Runbooks            | **IN PROGRESS**                         | ADR-001 **RECOMMENDED** Option A (Cloud Run); monitors retargeted to apex; scripts P0-020 present. **Blocked on signatures** for ACCEPTED/G9; `www` DNS cutover = Phase 15                                                            |
...
| P0-007 | Retarget health/Lighthouse monitors to canonical production   | 6, 10, 15   | G9              | [#186](https://github.com/redinc23/my_publishing/issues/186) — **agent DONE** (workflows); confirm on next scheduled run |
