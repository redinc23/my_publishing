...
| 5   | CI/CD Workflow Repair                             | **IN PROGRESS**                         | Format Check repaired (#211); P0-006 bug-to-issue trigger fixed (`CI` name); P0-005 fixed: playwright config honors `BASE_URL` (remote target, no local webServer), preview-e2e drops `USE_MOCKS`/placeholders; CI-confirm on next preview |
...
| 9   | Product Truth & Entitlement UX                    | **IN PROGRESS**                         | P0-012 (contact), P0-013 (newsletter), P0-014 (homepage stats) false-success surfaces fixed — honest env-gated behavior, 116/116 tests; G6 stays FALSE pending live Phase 12 QA + remaining public-claim/route-truth items                 |
