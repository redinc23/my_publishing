...
3. **Verify:** `./scripts/verify-gcp-production.sh` (liveness on `/api/live`, readiness on `/api/health?ready=1`).
4. **Vercel:** The legacy workflow is manual-only; treat it as an intentional preview/staging check, not production.
...
| Doc | Purpose |
|-----|---------|
| [docs/launch/ONE_PRODUCTION_BUILD.md](./launch/ONE_PRODUCTION_BUILD.md) | Single production build and release runbook |
| [docs/launch/PRODUCTION_ACCOUNT_LINKS.md](./launch/PRODUCTION_ACCOUNT_LINKS.md) | Safe workbook for dashboard hyperlinks, owners, and evidence |
