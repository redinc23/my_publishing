...
echo "â•‘  MANGU Publishers â€” Pre-Launch Verification                â•‘"
...
run_gate "validate-env" bash -c '
  set -euo pipefail
  if [[ ! -f .env.local ]]; then
    echo "No .env.local â€” using CI mock env"
  ...
  export NEXT_DISABLE_OUTPUT_FILE_TRACING=1
  fi
...
if [[ "${FAIL}" -gt 0 ]]; then
  echo "âŒ PRE-LAUNCH VERIFICATION FAILED"
  echo "Fix failing gates above, then re-run: bash scripts/pre-launch-verify.sh"
  exit 1
fi

echo "âœ… ALL LOCAL PRE-LAUNCH GATES PASSED"
echo ""
echo "Operator next steps (require real credentials â€” not run by this script):"
