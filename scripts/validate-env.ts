...
function loadProductionEnvironment(filePath: string) {
 ...
 */
function validateProductionShape(filePath: string) {
  const errors = [];
  const warnings = [];

  const required: Array<[string, (v: string) => boolean, string]> = [
    [
  ...
  const optional: Array<[string, (v: string) => boolean, string]> = [
  ...
  const modeOf = (v: string | undefined) => {
