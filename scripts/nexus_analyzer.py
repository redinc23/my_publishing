...
class NexusProjectAnalyzer:
    ...
    def _check_prompt_2(self) -> Dict:
        ...
        return {
            ...
            "has_auth": has_auth,
            "critical": True,
            "blocking": ["prompt_4_frontend", "prompt_5_deployment"]
            if status != "complete"
            else [],
        }
    ...
    def _check_prompt_4(self) -> Dict:
        ...
        has_next_config = (self.project_path / "next.config.js").exists() or (
    ...
    def assess_health_score(self) -> Dict:
        ...
        checks = {
            ...
            "has_tests": len(list(self.project_path.rglob("*.test.*"))) > 0
            or len(list(self.project_path.rglob("*.spec.*"))) > 0,
        ...
        # Deployment
        deploy_score = (
            sum([checks["has_docker"], checks["has_ci"], checks["has_env_example"]])
            / 3
            * 100
        )
...
"""
        ...
        if deps["nodejs"]["has_package_json"]:
            prompt += (
                f"**Node.js Packages:** {len(deps['nodejs']['dependencies'])} "
                f"dependencies, {len(deps['nodejs']['devDependencies'])} dev dependencies\n"
            )
...
# Visit http://localhost:3000 and test auth flow
...
"""
    ...
    def save_results(self, output_dir: str = "./nexus_analysis"):
        ...
        if self.results["analysis"]["structure"].get("empty_files"):
            print("   🧹 Cleanup script: cleanup.sh")
    ...
    def _write_summary(self, path: Path):
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            for key, value in health.items():
                ...
                f.write(
                    f"- {indicator} **{key.replace('_', ' ').title()}:** {value}/100\n"
                )
