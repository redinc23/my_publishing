      ...
      <Container>
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-secondary">{partner.institution_name} · {formatDate(order.created_at)}</p>
          </div>
          <div className="flex items-center gap-3">
            <Badge variant="outline">{titleCase(order.status)}</Badge>
            <form action={reorderPartnerOrder}>
              <input type="hidden" name="orderId" value={order.id} />
              <Button type="submit" variant="outline">Reorder</Button>
            </form>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-start justify-between gap-4 border-b border-border pb-4 last:border-0 last:pb-0">
                    <div>
                      ...
                      <p className="text-sm text-secondary">{item.book?.genre ?? 'Uncategorized'}</p>
                      {item.book ? (
                        <Link href={`/books/${item.book.slug}`} className="text-sm text-primary hover:underline">
                          View book
                        </Link>
                      ) : null}
                    </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <Link href="/partner/orders" className="inline-flex pt-2 text-primary hover:underline">
                Back to orders
              </Link>
            </CardContent>
          </Card>
